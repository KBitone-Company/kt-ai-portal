...
국방지능화플랫폼 (K-Defense Intelligence Platform)의 메인 화면이다. 사용자가 Backstage 포털에 로그인 후 처음 보는 화면으로, 데이터셋/AI 모델/프로젝트/반출승인/크레딧/K-RMF 증빙 등 전체 기능을 한눈에 파악할 수 있도록 구성한다.
...
- 제목: `국방지능화플랫폼`
- 부제: `K-Defense Intelligence Platform`
...
| 메뉴 | 이동처 | 유형 |
|------|--------|------|
| Admin Console | `/admin-console` | 내부 |
